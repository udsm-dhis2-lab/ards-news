angular.module('multi-select-tree').run(['$templateCache', function($templateCache) {
    ...
    $templateCache.put('src/multi-select-tree.tpl.html',
        ...
        "    </span>\n" +
        "    <span ng-if=\"selectedItems.length > 0\" class=\"selected-items\">\n" +
        "      <span ng-if=\"selectedItems.length <=3\" ng-repeat=\"selectedItem in selectedItems\" class=\"selected-item\">{{selectedItem.name}} <span class=\"selected-item-close\"\n" +
        "                                                                                  ng-click=\"deselectItem(selectedItem, $event)\"></span></span>\n" +
        "        <span class=\"caret\"></span>\n" +
        "      <span    ng-if=\"selectedItems.length > 3\" ng-bind=\"maximumLabel\" class=\"selected-item\"  ></span>\n" +
        "    </span>\n" +
